...
**Next:** Phase 9 (Read-Only Edge Engine) planned for v1.2 — `ImmutableEngine` for serverless deployments via Rust. WASM bindings are out of scope (D22). See `.planning/phases/09-readonly-edge/09-00-PLAN.md` and `SCALING.md`.
...
| Text index | Single merged inverted index per namespace (key `__index__`), incrementally updated on write. O(1) search read. **Crash recovery:** sidecar carries LSN watermark; stale sidecars are automatically rebuilt from raw records on `Engine::open()`. |
